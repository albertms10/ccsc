...
import { closestTimeValue } from '../../utils/utils';
...
export default ({ socis, getSocis, loading }) => {
  ...
  useEffect(() => {
    const filteredSocis = socis.filter(soci => {
      return soci.nom_complet.toLowerCase().includes(searchValue)
  ...
  const handleEliminar = (id) => {
    fetch(`/api/socis/${id}`, { method: 'DELETE' })
      .then(() => getSocis());
  };

  const showDeleteConfirm = (id) => {
    Modal.confirm({
      title: 'Confirmes que vols eliminar el soci i totes les dades associades?',
      ...
      <Table
        ...
        columns={[
            ...
            render: (text, row) => (
              <>
                ...
                <div style={{ display: 'inline-grid' }}>
                  ...
                  <Text type="secondary" style={{ marginBottom: 0 }}>{row.username}</Text>
                </div>
              </>
            ),
          ...
          },
          {
            ...
            dataIndex: 'email',
            key: 'email',
            render: text => <Paragraph className="action-text" copyable>{text}</Paragraph>,
          },
          {
            title: 'Telèfon',
            dataIndex: 'telefon',
          ...
          },
          {
            ...
            render: (estat_actiu, row) => (
              <Tooltip
                title={`${closestTimeValue(row.dies_activitat, 'd')} d’${row.data_inactiu ? 'in' : ''}activitat`}>
                {!estat_actiu || row.data_inactiu ? <Badge status="error" text="Inactiu" /> :
